    ...
    if (!check) {
        ...
        try {
            const response = await openai.chat.completions.create({
                ...
                messages: [{ role: "user", content: `Here is the list of questions and answers 
                    that the user answered in a career quiz ${prompt}. Notice that a 0 = strongly disagree, 1 = disagree, 2 = neutral, 3 = agree, 4 = strongly agree. Please respond with three 
                    viable career options in the form of a JSON array. Make sure the response is only JSON array and no other text. Make sure the three careers are within the same industry as the one provided: ${industry}.
                    Object must contain these fields:
                    - title: The career title
                    - description: A short description of the career
                    - traits: 3-5 personality traits that align with this career
